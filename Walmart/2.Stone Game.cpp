class Solution {
public:
    // as n is even then always Alice will win the game
    int n;
    int t[501][501];
    int solve(vector<int>& piles, int left, int right) 
    {
        ...
        */
        return t[left][right] = max(piles[left]  + min(solve(piles, left+2, right), solve(piles, left+1, right-1)), piles[right] + min(solve(piles, left, right-2), solve(piles, left+1, right-1)));
